# Simple Timesheet Server Launcher
...
function Start-TimesheetServer {
    Write-ColorOutput "Starting Timesheet Server..." "Green"
    ...
    # Check if timesheet directory exists
    if (-not (Test-Path $timesheetPath)) {
        Write-ColorOutput "ERROR: Timesheet directory not found at: $timesheetPath" "Red"
        return
    }
    
    Write-ColorOutput "Workspace: $workspaceRoot" "Cyan"
    Write-ColorOutput "Timesheet Path: $timesheetPath" "Cyan"
    Write-Host ""
    
    # Navigate to timesheet directory
    Set-Location $timesheetPath
    Write-ColorOutput "Current Directory: $(Get-Location)" "Yellow"
    
    # Check if virtual environment exists
    $venvPath = Join-Path $timesheetPath "venv"
    ...
    $managePath = Join-Path $timesheetPath "manage.py"
    ...
    try {
        ...
        Write-ColorOutput "Timesheet server stopped" "Yellow"
    }
}

# Main execution
Start-TimesheetServer
